    ...
    <?php if( ! empty( $args->wishlisted_events ) && count( $args->wishlisted_events ) ) {?>
        ...
        <?php foreach( $args->wishlisted_events as $event_data ) {
            if( isset( $event_data['event'] ) && ! empty( $event_data['event'] ) && isset( $event_data['event']->name ) ) {
                if(!isset($event_data['event']->image_url) || empty($event_data['event']->image_url))
                {
                    $event_data['event']->image_url = plugin_dir_url(EP_PLUGIN_FILE).'public/partials/images/dummy_image.png';
                }
                ?>
                <div class="ep-box-row ep-my-booking-row ep-border ep-rounded ep-overflow-hidden ep-text-small ep-mb-4" id="ep_profile_wishlist_event_<?php echo esc_attr($event_data['event']->id);?>">
                    ...
                    <div class="ep-box-col-6 ep-ps-4 ep-d-flex ep-align-items-center ep-justify-content-between ep-bg-white">
                        <div>
                    ...
                    <div class="ep-box-col-4 ep-d-flex ep-justify-content-between ep-align-items-center ep-bg-white">
                        <div class="">
                        ...
                        <div class="ep-text-end">
                            <div class="ep-btn-group">
                                <a href="javascript:void(0);" class="ep-btn ep-text-danger ep-event-action ep_event_wishlist_action ep-px-2" id="ep_event_wishlist_action_<?php echo esc_attr( $event_data['event']->id );?>" data-event_id="<?php echo esc_attr( $event_data['event']->id );?>" data-remove_row="ep_profile_wishlist_event_<?php echo esc_attr( $event_data['event']->id );?>" title="<?php echo esc_attr( $ep_functions->ep_global_settings_button_title( 'Remove From Wishlist' ) );?>">
