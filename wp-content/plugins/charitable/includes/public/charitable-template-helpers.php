...
function charitable_template_from_session_content( $template_key, $wrapper_args = array(), $default = '' ) {
	...
	$content  = '<div class="charitable-session-content" data-template="' . esc_attr( $template_key ) . '" data-args="' . esc_attr( wp_json_encode( $wrapper_args ) ) . '" style="display: none;">' . $default . '</div>';
 ...
 *
 * @since  1.4.11
 * @since  1.8.1 added 'charitable_is_main_loop' filter.
 *
 * @return boolean
 */
function charitable_is_main_loop() {
	$is_main_loop = is_single() && in_the_loop() && is_main_query();
	return apply_filters( 'charitable_is_main_loop', $is_main_loop );
}
